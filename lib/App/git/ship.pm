...
sub init {
  ...
  if (@_ and ref($self) eq __PACKAGE__) {
    $self->abort("Cannot create new project from $_[0]");
  }
  else {
    $self->_generate_config;
    $self->_generate_gitignore;
    $self->system(qw( git init )) unless -d '.git' and @_;
    $self->system(qw( git add .gitignore ), $self->_config_file);
  }
...
sub _generate_config {
  ...
  print $CFG "# Generated by git-ship. See 'git-ship --man' for help or http://https://metacpan.org/pod/App::git::ship::Manual\n";
  print $CFG "class = $class\n";
  print $CFG "project_name = \n";
  print $CFG "homepage = $homepage\n";
  print $CFG "bugtracker = " . +(join '/', $homepage, 'issues') =~ s!(\w)//!$1/!r . "\n";
  print $CFG "license = artistic_2\n";

  say "git-ship: Created config file $config_file";
}
